<form action="{{ route('capsule-post.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    ...
    <div>
        ...
        <x-text-input id="title" name="title" type="text" class="mt-1 block w-full"
            placeholder="Enter your capsule title (Title will be shown on the capsule cover)" :value="old('title')" required
            autofocus />
    ...
    <div>
        ...
        <textarea id="text" name="text" rows="4"
            ...
            placeholder="Write your capsule content here..." required>{{ old('text') }}</textarea>
    ...
    <div>
        ...
        <div class="mt-1">
            <label
                class="flex justify-center px-6 py-4 border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-indigo-500 focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
    ...
    <div>
        ...
        <input type="datetime-local" name="future_time" id="future_time"
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm px-2 py-1.5 border"
            value="{{ old('future_time') }}" min="{{ now()->format('Y-m-d\TH:i') }}" required>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        ['dragenter', 'dragover'].forEach(eventName => {
            uploadLabel.addEventListener(eventName, () => {
                uploadLabel.classList.add('border-indigo-500', 'ring-2', 'ring-offset-2',
                    'ring-indigo-500');
            });
        });
        ...
        ['dragleave', 'drop'].forEach(eventName => {
            uploadLabel.addEventListener(eventName, () => {
                uploadLabel.classList.remove('border-indigo-500', 'ring-2', 'ring-offset-2',
                    'ring-indigo-500');
            });
        });
